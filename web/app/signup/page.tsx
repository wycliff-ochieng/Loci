"use client";

import Link from "next/link";
import { useRouter } from "next/navigation";
import { useState } from "react";
import { TopBar } from "../../components/TopBar";
import { login, register } from "../../lib/api";

export default function SignupPage() {
  const [form, setForm] = useState({ email: "", password: "", username: "" });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const router = useRouter();

  const handleSignup = async () => {
    setBusy(true);
    setError("");
    try {
      if (!form.email || !form.password || !form.username) {
        setError("All fields are required");
        return;
      }

      await register({ email: form.email, password: form.password, username: form.username });
      await login(form.email, form.password, form.username);
      router.push("/map");
    } catch (e) {
      const msg = e instanceof Error ? e.message : "Sign up failed. Try again.";
      setError(msg);
    } finally {
      setBusy(false);
    }
  };

  return (
    <div className="layout-shell" style={{ minHeight: "100vh" }}>
      <TopBar />
      <main
        style={{
          display: "grid",
          placeItems: "center",
          padding: "32px 18px 64px",
          background: "radial-gradient(circle at 50% 10%, rgba(47,123,255,0.08), transparent 42%), var(--bg)",
        }}
      >
        <div
          className="panel"
          style={{ maxWidth: 420, width: "100%", display: "grid", gap: 12, padding: 22, boxShadow: "0 20px 50px rgba(42,18,47,0.08)" }}
        >
          <div>
            <div className="badge" style={{ border: "1px solid var(--accent)", color: "var(--accent)" }}>
              New here
            </div>
            <h2 style={{ margin: "6px 0 0" }}>Create your account</h2>
            <p className="muted" style={{ margin: 0 }}>Drop messages on the map, reply, and explore what's nearby.</p>
          </div>
          <label style={{ display: "grid", gap: 6 }}>
            <span className="muted">Username</span>
            <input
              value={form.username}
              onChange={(e) => setForm({ ...form, username: e.target.value })}
              placeholder="yourname"
              style={inputStyle}
            />
          </label>
          <label style={{ display: "grid", gap: 6 }}>
            <span className="muted">Email</span>
            <input
              value={form.email}
              onChange={(e) => setForm({ ...form, email: e.target.value })}
              placeholder="you@example.com"
              style={inputStyle}
              type="email"
            />
          </label>
          <label style={{ display: "grid", gap: 6 }}>
            <span className="muted">Password</span>
            <input
              value={form.password}
              onChange={(e) => setForm({ ...form, password: e.target.value })}
              placeholder="••••••••"
              style={inputStyle}
              type="password"
            />
          </label>
          {error && <div style={{ color: "var(--danger)", fontWeight: 600 }}>{error}</div>}
          <button className="primary-btn" onClick={handleSignup} disabled={busy}>
            {busy ? "Creating..." : "Sign up"}
          </button>
          <div className="muted" style={{ fontSize: 14 }}>
            Already have an account? <Link href="/login" className="badge" style={{ border: "1px solid var(--accent)", color: "var(--accent)" }}>Login</Link>
          </div>
        </div>
      </main>
    </div>
  );
}

const inputStyle: React.CSSProperties = {
  background: "#fff",
  border: "1px solid var(--border)",
  borderRadius: 12,
  padding: "12px 14px",
  color: "var(--text)",
};
